...
To further see if there were any correlations, a chi-square and correlation test was done. For the chi-square, the lowest was p-value was 0.344. This was a correlation between the country versus the corruption score. This p-value is still high and not low enough. Typically, in order for there to be an actual correlation between the variables, the p-value has to be lower than 0.05. The issue with chi-square is it only relies on two variables. More than two variables can not be done with this test. Below is a visualization using ggcorr function from the GGally package in R. 

![Correlation Plot](GGally_cor.png)
